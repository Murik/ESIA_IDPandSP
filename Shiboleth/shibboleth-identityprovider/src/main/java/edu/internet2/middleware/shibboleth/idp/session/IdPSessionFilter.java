...
public class IdPSessionFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException,
        ...
        Session idpSession = getUserSession(sessionCookie, httpRequest, (HttpServletResponse) response);
        if (idpSession != null) {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        cookieDomain = HttpServletHelper.getCookieDomain(filterConfig.getServletContext());
    ...
    protected Session getUserSession(Cookie sessionCookie, HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
        ...
        // index 0: V4 remote address
        // index 1: V6 remote address
        // index 2: session ID
        // index 3: Base64(HMAC(index 0 + index 1 + index 2))
        ...
        byte[] v4AddressBytes = Base64.decode(valueComponents[0]);
        byte[] v6AddressBytes = Base64.decode(valueComponents[1]);
        byte[] sessionIdBytes = Base64.decode(valueComponents[2]);
        byte[] signatureBytes = Base64.decode(valueComponents[3]);

        if (v4AddressBytes == null || v6AddressBytes == null || sessionIdBytes == null || signatureBytes == null) {
            log.warn("A session cookie field was null");
            return null;
        }
        ...
        if (userSession != null) {
            if (isCookieValid(httpRequest, httpResponse, v4AddressBytes, v6AddressBytes, sessionIdBytes, signatureBytes, userSession
                    .getSessionSecret())) {
                return userSession;
            }
        } else {
            log.debug("No session associated with session ID {} - session must have timed out", sessionId);
        }
        return null;
    }
     ...
     * @param v4AddressBytes IPv4 remote address from the cookie value
     * @param v6AddressBytes IPv6 remote address from the cookie value
     ...
     */
    protected boolean isCookieValid(HttpServletRequest httpRequest, HttpServletResponse httpResponse,
            byte[] v4AddressBytes, byte[] v6AddressBytes, byte[] sessionIdBytes, byte[] signatureBytes,
            byte[] sessionSecret) {
        
        boolean isV4 = false;
        String sessionAddress = null, clientAddress = null;
        ...
        if (consistentAddress && sessionAddress.equals("-")) {
            ...
            // Recompute signature with new address populated.
            ...
            log.debug("Updating IdP session cookie via HTTP response");
            StringBuilder cookieValue = new StringBuilder();
            cookieValue.append(Base64.encodeBytes(v4AddressBytes, Base64.DONT_BREAK_LINES)).append("|");
            cookieValue.append(Base64.encodeBytes(v6AddressBytes, Base64.DONT_BREAK_LINES)).append("|");
            cookieValue.append(Base64.encodeBytes(sessionIdBytes, Base64.DONT_BREAK_LINES)).append("|");
            cookieValue.append(signature);

            Cookie sessionCookie = new Cookie(AuthenticationEngine.IDP_SESSION_COOKIE_NAME,
                    HTTPTransportUtils.urlEncode(cookieValue.toString()));
