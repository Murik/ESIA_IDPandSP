...
public abstract class AbstractSAML1ProfileHandler extends AbstractSAMLProfileHandler {
    ...
    protected Conditions buildConditions(BaseSAML1ProfileRequestContext<?, ?, ?> requestContext, DateTime issueInstant) {
        ...
        // Include NotBefore condition if includeConditionsNotBefore() returns true
        if (profileConfig.includeConditionsNotBefore()) {
     ...
     * @return  a usable session index based on the input
     */
    ...
    protected void writeAuditLogEntry(BaseSAMLProfileRequestContext context) {
        ...
        if (context.getMessageEncoder() != null) {
        ...
        } else if (context.getSubjectNameIdentifier() != null && context.getSubjectNameIdentifier() instanceof NameIdentifier) {
            auditLogEntry.setNameIdValue(((NameIdentifier) context.getSubjectNameIdentifier()).getNameIdentifier());
        }

        getAuditLog().info(auditLogEntry.toString());
    }
