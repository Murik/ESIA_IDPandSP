    ...
    public void processRequest(HTTPInTransport inTransport, HTTPOutTransport outTransport) throws ProfileException {
        ...
        try {
            ...
            if (requestContext.getProfileConfiguration() == null) {
                ...
                // We're adding support for attribute filtering, but we still don't support value filtering.
                ...
                if (query.getAttributes() != null) {
                    for (Attribute desig : query.getAttributes()) {
                        if (desig.getAttributeValues() != null && !desig.getAttributeValues().isEmpty()) {
                            log.warn("Specific attribute values requested in query from '{}'. This functionality is not supported",
                                    requestContext.getInboundMessageIssuer());
                            requestContext.setFailureStatus(buildStatus(StatusCode.RESPONDER_URI,
                                    StatusCode.REQUEST_UNSUPPORTED_URI,
                                    "Request of specific attribute values during an attribute query is not supported"));
