...
}function o(z){while(!i[z]&&--z>=0){}for(var y=z;
y<d;
...
}else{if((x>=32&&x<=125)||x>186){var z=t(C.begin-1);
...
}if(!u.attr("readonly")){u.one("unmask",function(){u.unbind(".mask").removeData("buffer").removeData("tests")
}).bind("focus.mask",function(){w=u.val();
var x=n();
r();
setTimeout(function(){if(x==f.length){u.caret(0,x)
}else{u.caret(x)
}},0)
